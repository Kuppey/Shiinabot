...
def tag_clean(tag_html):
    text = tag_html.find('a', class_="search-tag")
    text = text.text
    text = text.split("(?)")
    text = text[0]
...
def get_image_online(**kwargs):
    if kwargs.get('used images'):
        ...
        txt_name = os.path.join(os.getcwd(), "Used danbooru {0}.txt".format(
            kwargs['bot name']))
        try:
            used_links = open(txt_name, 'r').read().splitlines()
        except:
    ...
    if utils.is_bool(kwargs.get('login')):
        ...
        url_login = "https://danbooru.donmai.us/session/new"
        form_num = 0
        form_user = "name"
    ...
    if kwargs.get('tags'):
        if not utils.is_bool(kwargs.get('login')):
            if len(kwargs.get('tags').split(', ')) > 2:
                print("!WARNING! You can not have more than 2 tags")
                print("while under a free account with Danbooru!")
        ...
        while True:
            while True:
                ...
                for taga in site_tag:
                    tag = tag_clean(taga)
                    if taga['class'][0] == "category-1":
                        art_tags.append(tag.title())
                    elif taga['class'][0] == "category-3":
                        sers_tags.append(tag.title())
                    elif taga['class'][0] == "category-4":
            ...
            try:
                url = "{0}{1}".format(url_start, image_url['src'])
            except:
                # Flash File
                continue
            filename = ""
            if not kwargs.get('message'):
                message = ""
            sn_kwgs = {}
            sn_url, sn_kwgs = utils.saucenao(url,
                                             kwargs.get('saucenao api'), True)
            re_dict = {'{#artist}': (
